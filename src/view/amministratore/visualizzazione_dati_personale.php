...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['ricercaPersonale'])) {
    ...
    UNION SELECT nBadge, nome, cognome, tipologia, CF, emailAziendale, dataNascita, luogoNascita, recapitoTelefonico, inizioRapporto, fineRapporto FROM medico WHERE (nome LIKE '%$search_term%' OR cognome LIKE '%$search_term%' OR 
    CF LIKE '%$search_term%' OR nBadge LIKE '%$search_term%') 
...
<html lang="en">
  ...
  <body>
    <div class="container">
        ...
        <h4>Attualmente sono in impiego: <?php echo $numeri_personale[0]['totale']?> dipendenti</h4>

        <div class="quantita_personale">
            <div class="medici_qt">
                <p>Medici: <?php echo $numeri_personale[0]['medici']?></p>
            </div>
            <div class="receptionist_qt">
                <p>Receptionist: <?php echo $numeri_personale[0]['receptionist']?></p>
            </div>
            <div class="operatori_qt">
                <p>Operatori: <?php echo $numeri_personale[0]['operatori']?></p>
            </div>
        </div>
